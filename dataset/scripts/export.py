...
def fmt_date(val):
    ...
    # Formato "Thu May 14" (Excel auto-format sin año)
    for fmt in ("%a %b %d", "%A %B %d"):
        try:
            parsed = datetime.strptime(s, fmt)
            year = date.today().year
            d = parsed.replace(year=year)
            # Si la fecha queda en el futuro, pertenece al año anterior
            if d.date() > date.today():
                d = parsed.replace(year=year - 1)
